...
form caption("Trapped12") size(700, 330), guiMode("queue") pluginId("rb12")

button  bounds(56, 132, 80, 42) channel("trigger1") text("Trigger") textColour("white")

checkbox bounds(138, 182, 27, 25), channel("reTrigger"), fontColour("white"),  value(1)
label    bounds(6, 182, 114, 22), text("ReTrigger"), fontColour(255, 255, 255, 255)

hslider bounds(16, 72, 175, 50) channel("dur") range(.005, 2, 1, 1, 0.001) text("Dur") textColour("white")

hslider bounds(4, 228, 175, 50) channel("rate") range(.33, 18, 1, 1, 0.001) text("ReTrig Rate") textColour("white")
...
hslider bounds(252, 68, 160, 52) channel("frq") range(100, 4000, 1300, 1, 0.001) text("Freq") textColour("white")
hslider bounds(428, 68, 150, 50) channel("rndFrq") range(1, 4, 2, 1, 0.001) text("RandFreq") textColour("white")

hslider bounds(200, 130, 161, 51) channel("fltStrt") range(50, 2000, 100, 1, 0.001) text("FilterStart") textColour("white")
hslider bounds(360, 128, 161, 51) channel("fltPeak") range(50, 2000, 500, 1, 0.001) text("FilterPeak") textColour("white")
...
hslider bounds(426, 188, 150, 50) channel("rvbPan") range(.001, 6, 11, 1, 0.001) text("Rvb Pan") textColour("white")
...
ip3          =         chnget:i("dur")
ip5          =         chnget:i("synthLvl")             
ip4           =         chnget:i("frq")*rnd(chnget:i("rndFrq"))
...
k1             linseg    0, ip3 * .8, 8, ip3 * .2, 0     
k2             phasor    k1                         
k3             table     k2 * ifuncl, 22                    

kGate          transeg   1, ip3, 0, 0
kCut           transeg   iFltStrt, ip3 * .7, 1, iFltPeak, ip3 * .3, iFltStrt * .9, 6

anoise         rand      10                     
aFlt           butterbp  anoise * chnget:k("noiseLvl") * .6, 20 + (kCut + ip4), 20 + (kCut * iFltBW), 1
aSig           oscil     kGate * ip5, ip4 + k3, 1
aMix           =         kGate * (aSig + aFlt) * .5
